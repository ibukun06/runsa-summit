      ...
      {!showPin ? (
        ...
        <div>
          <div style={{ fontFamily:"'Cinzel', serif", fontSize:14, fontWeight:700,
            color: T.dark ? BRAND.goldLight : BRAND.navyDark, marginBottom:6 }}>
            Enter Check-In PIN to {checkinOpen ? "close" : "open"} check-in
          </div>
          <div style={{ display:"flex", gap:10, marginTop:10, flexWrap:"wrap" }}>
            <input
              ...
              maxLength={6}
              autoFocus
              style={{ ...inputStyle(T, pinErr), width:140, textAlign:"center",
                letterSpacing:"0.25em", fontSize:20 }}
              placeholder="——————"
              value={pin}
            ...
            <button onClick={handleCancel} style={{
              ...
              border:`1px solid ${T.border}`, background:"transparent",
              color:T.textMuted, fontSize:13, fontFamily:"'Inter', sans-serif" }}>
              Cancel
            </button>
          </div>
          {pinErr && (
...
function AdminView({ regs, onReset, onDeleteDelegate, checkinOpen, onToggleCheckin, T }) {
  ...
  const downloadXLSX = () => {
    // Uses HTML table exported as .xls — Excel and LibreOffice open this natively
    // with no ZIP corruption issues. Styles applied via inline CSS that Excel reads.
    const esc = s => String(s ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    const rows = regs.map(r => [
      r.id,
      r.name || '',
      normalizeInstitution(r.institution) || '',
      r.level || '',
      r.position || '',
    ...
    const hdrStyle = 'background:#0D1E38;color:#F5D57A;font-weight:bold;font-family:Calibri;font-size:11pt;text-align:center;border:1px solid #888;padding:4px 8px;';
    const cellStyle = 'font-family:Calibri;font-size:10pt;border:1px solid #D0D0D0;padding:3px 6px;';
    const cellAlt   = 'background:#F1F8E9;font-family:Calibri;font-size:10pt;border:1px solid #D0D0D0;padding:3px 6px;';
    const yesStyle  = 'background:#F1F8E9;color:#2E7D32;font-weight:bold;font-family:Calibri;font-size:10pt;border:1px solid #D0D0D0;padding:3px 6px;text-align:center;';
    const noStyle   = 'background:#FFEBEE;color:#C62828;font-weight:bold;font-family:Calibri;font-size:10pt;border:1px solid #D0D0D0;padding:3px 6px;text-align:center;';

    const headers = ['Ticket ID','Full Name','Institution','Level','Position','Registered','Checked In','Check-In Time'];

    const hdrRow = '<tr>' + headers.map(h => `<th style="${hdrStyle}">${h}</th>`).join('') + '</tr>';
    const dataRows = rows.map((row, ri) => {
      const bg = ri % 2 === 0 ? cellStyle : cellAlt;
      return '<tr>' + row.map((cell, ci) => {
        if (ci === 6) {
          return `<td style="${cell === 'Yes' ? yesStyle : noStyle}">${esc(cell)}</td>`;
        }
        return `<td style="${bg}">${esc(cell)}</td>`;
      }).join('') + '</tr>';
    }).join('');
...
<table border="1" style="border-collapse:collapse;font-family:Calibri;">
${hdrRow}${dataRows}
</table></body></html>`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `RUNSA-Delegates-${new Date().toISOString().slice(0, 10)}.xls`;
    a.click();
    URL.revokeObjectURL(url);
  };
